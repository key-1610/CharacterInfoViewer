...
const BASIC_STATUS = [
    { label: "HP", min: 6, max: 18 },
    { label: "MP", min: 3, max: 18 },
    { label: "SAN", min: 0, max: 99 },
    { label: "DB", min: "-1d6", max: "+1d6" },
    { label: "イニシアティブ", min: 3, max: 18 }
];

const BASIC_PARAMS = [
    { label: "STR", min: 3, max: 18 },
    { label: "CON", min: 3, max: 18 },
    { label: "POW", min: 3, max: 18 },
    { label: "DEX", min: 3, max: 18 },
    { label: "APP", min: 3, max: 18 },
    { label: "SIZ", min: 8, max: 18 },
    { label: "INT", min: 8, max: 18 },
    { label: "EDU", min: 6, max: 21 }
];
...
function getDamageBonus(str, siz) {
    ...
    // 40を超えた場合、16ごとに+1D6増加
    const extra = Math.floor((total - 40) / 16);
    const bonusDice = 2 + extra;
    return `+${bonusDice}D6`;
